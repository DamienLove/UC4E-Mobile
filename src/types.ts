...
export interface GameNode {
  id: string;
  label: string;
  type: string; // e.g., 'star', 'rocky_planet', 'player_consciousness', 'asteroid'
  x: number;
  y: number;
  vx: number;
  vy: number;
...
// New Interface for hittable dust
...
export interface EnergyOrb {
  ...
  radius: number;
  isFromBloom?: boolean;
  value: number; // Added value for varying orb worth
}
...
export interface GameState {
  ...
  turnHitIds: Set<string>; // Track nodes hit this turn for combos
  ...
  visualEffects: VisualEffect[]; // General purpose visual effects (flares, sparks)
...
export type NodeType = 'star' | 'rocky_planet' | 'life_seed' | 'sentient_colony' | 'player_consciousness' | 'asteroid';
