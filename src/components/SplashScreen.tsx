...
const SplashScreen: React.FC<SplashScreenProps> = ({ onStartGame, onLoadGame, dispatch, settings }) => {
  ...
  const handleStartGameWithMusic = async () => {
      // Check for API key selection for Veo/Imagen features
      if ((window as any).aistudio) {
          ...
          if (aistudio.hasSelectedApiKey && aistudio.openSelectKey) {
              ...
              if (!hasKey) {
                  ...
                  // Race condition: assume key selection was successful and proceed immediately
              }
          }
      }
  ...
  const handleLoadGameWithMusic = async () => {
      // Check for API key selection for Veo/Imagen features
      if ((window as any).aistudio) {
          const aistudio = (window as any).aistudio;
          if (aistudio.hasSelectedApiKey && aistudio.openSelectKey) {
