...
import { GameState, GameAction, Upgrade, EnergyOrb, GameNode, QuantumPhage, CollectionEffect, CosmicEvent, AnomalyParticle, ConnectionParticle, WorldTransform, ProjectionState, CollectedItem, Satellite, SpaceDust, Shockwave } from '../types';
...
const MAX_LAUNCH_POWER = 35; // Increased for more speed
const PROJECTILE_FRICTION = 0.985;
const REFORM_DURATION = 120; 

const ORB_COLLECTION_LEEWAY = 20; 
...
function gameReducer(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    case 'START_GAME': {
      ...
      // Initialize dust
      const initialDust = createSpaceDust(60, 2000, 2000);
    ...
    case 'TICK': {
            ...
            case 'PROJECTING': {
              ...
              nextState.spaceDust.forEach(dust => {
                  ...
                  if (dist < playerNode!.radius + dust.size + 10) {
                      ...
                      // Could add a tiny pop sound here if audio service supports rapid fire
                  }
              });
              ...
              for (const node of mutableNodes) {
                  ...
                  if (dist < playerNode.radius + node.radius) {
                      ...
                      if (node.type === 'star') {
                          ...
                          const bounceSpeed = Math.max(speed * 1.5, 15); // Accelerate!
                          
                          playerNode.vx = Math.cos(angle) * bounceSpeed;
                          playerNode.vy = Math.sin(angle) * bounceSpeed;
                          
                          // Massive reward
                          nextState.energy += 250;
                          
                          // Visuals
                          nextState.screenShake = { intensity: 25, duration: 20 };
                          newShockwaves.push({ id: `shock_${Date.now()}`, x: node.x, y: node.y, maxRadius: 300, life: 0 });
                          audioService.playSound('phage_spawn'); // Boom sound
                          ...
                          for(let i=0; i<8; i++) {
                              ...
                              nextState.energyOrbs.push({
                                  ...
                                  x: node.x, y: node.y,
                                  vx: Math.cos(orbAngle) * 5, vy: Math.sin(orbAngle) * 5,
                                  radius: 8, value: 50
                              });
                          }
                          ...
                          // ASSIMILATION EVENT
                          // Stop player momentarily or slow them down? Let's slow them to show impact.
                          playerNode.vx *= 0.5;
                          playerNode.vy *= 0.5;
                          
                          // Remove from world, add to satellite
                          nextState.nodes = nextState.nodes.filter(n => n.id !== node.id);
                          mutableNodes = mutableNodes.filter(n => n.id !== node.id); // Update local ref
                          ...
                          nextState.biomass += 50;
                          nextState.screenShake = { intensity: 10, duration: 10 };
                          newShockwaves.push({ id: `shock_${Date.now()}`, x: node.x, y: node.y, maxRadius: 100, life: 0 });
                          audioService.playSound('connection_bounce'); // Cracking sound
                          nextState.notifications.push(`Assimilated ${node.label}`);
                      }
              ...
              // Reform if too slow
              if (Math.hypot(playerNode.vx, playerNode.vy) < 0.5) {
                  playerNode.vx = 0;
                  playerNode.vy = 0;
      ...
      if (playerNode) {
          nextState.satellites = nextState.satellites.map(sat => {
              ...
              // Passive income from satellites
              nextState.energy += 0.05; 
              if (sat.type === 'life_seed') nextState.biomass += 0.05;
              return sat;
          });
      }
    ...
    case 'COMPLETE_LEVEL_TRANSITION': {
       ...
       return { 
         ...
         energyOrbs: [],
         cosmicEvents: [],
         spaceDust: createSpaceDust(50, 2000, 2000), // Refresh dust
       };
    }
